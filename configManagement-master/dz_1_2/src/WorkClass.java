...
public class WorkClass {
    ...
    public static void main(String args[])
    {
        ...
        try {
            link = downloadHtml(inputString);//Получаем список версий для заданного пакета
            if(link!=null && link.length!=0)
                chosenLink = choiceVersion(link);//Выброр нужной версии
            else
            {
            ...
            rec(inputString,dependences,2);//Рекурсивная функция для поиску зависимостей
            Graph.close();
            Graph.showGraph();
            delete();//Очистка памяти
    ...
    private static Link[] downloadHtml(String packageName) {
        try {
            ...
            Document document;
            //Получение html
            document = Jsoup.connect(urlStr+packageName+"/").get();
            //Распарсировка html
            Elements elements = document.select("a:contains(.whl)");
            Link[] link = new Link[elements.size()];
            ...
            {
                link[i].setIndex(i+1);
                link[i].setName(elements.get(i).text());
                link[i].setUrl(new URL(elements.get(i).attr("href")));
                Pattern pattern = Pattern.compile("-\\w+\\.\\w+\\.\\w+|-\\w+\\.\\w+|-\\w+");
                Matcher matcher = pattern.matcher(elements.get(i).text());
                if(matcher.find())
                    link[i].setVersion(elements.get(i).text().substring(matcher.start()+1,matcher.end()));
                else
                    link[i].setVersion("");
            }
            return link;
